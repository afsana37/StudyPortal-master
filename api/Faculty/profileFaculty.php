...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (empty($_POST['st_old_password']) || empty($_POST['st_password'])
        ...
        <script>
            ...
            window.location.href='../../index_teacher.php#profile';
        </script>
        <?php
    }
    else
    {
        ...
        $query1 = $con->query("select faculty_password from faculty where faculty_username = '$faculty_username'");
        ...
        if (mysqli_num_rows($query1) > 0) {    // whether we found any row after the query or not.
            ...
            while ($row = mysqli_fetch_array($query1, MYSQLI_BOTH)) {   // taking each row of the resultset of the query.
                ...
                $old_pass = $row['faculty_password'];

            }
            if ($old_pass==$st_old_password){

                if($st_password==$st_password_again){
                    $query2 = $con->query("update faculty set faculty_password='$st_password' where faculty_username='$faculty_username'");
                    if($query2){
                        ?>
                        <script>
                            alert('Password changed successfully');
                            window.location.href='../../index_teacher.php';
                        </script>
                        <?php
                    }
                    else{
